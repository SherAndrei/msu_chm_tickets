...
\begin{theorem}
  ...
  \begin{flalign*}
    ...
    & \norm{A}_2 = \sqrt{\max_i \lambda_i (A^\intercal A)} - \text{корень из максимального собственного числа матрицы } A^\intercal A
  \end{flalign*}
...
\begin{lemma}
  ...
  \begin{proof}
    \begin{flalign*}
      \eqref{eq:stable_cond}\quad & \cnd(\alpha A) = \norm{\alpha A} \norm{\alpha^{-1} A^{-1}} = \frac{\abs{\alpha}}{\abs{\alpha}}\cnd(A) \\
      \eqref{eq:positive_cond}\quad & \cnd(A) = \norm{A}\norm{A^{-1}} \ge \norm{A A^{-1}} = \norm{I} \ge 1 \\
      \eqref{eq:cond_two}\quad & \cnd_2(A)=\norm{A}\norm{A^{-1}}=\max_i\lambda_i(A)\cdot\max_i\lambda^{-1}_i(A)=\frac{\max_i\lambda_i(A)}{\min_i\lambda_i(A)}
    \end{flalign*}
    \begin{multline*}
